...
export default class ObjectMapping extends React.Component {
	constructor(props) {
		super(props);
	...
	onchangeObjectSelection(val, event) {
		...
		if (
			...
			this.state.ObjectDesb[event.target.value] == undefined
		) {
			var call = false;
		...
		if (this.state && this.state.ObjectDesb != undefined) {
			for (var i = 0; i < Object.keys(this.state.ObjectDesb).length; i++) {
				...
				for (var j = 0; j < this.state.ObjectDesb[Object.keys(this.state.ObjectDesb)[i]].fields.length; j++) {
					if (
						this.state.ObjectDesb[Object.keys(this.state.ObjectDesb)[i]].fields[j].externalId == true ||
			...
			<div className="slds-card">
				<table className="slds-table">
					<tr>
						...
						<th>
							<label className="slds-text-heading_medium">External Id From Sheet</label>
						</th>
						<th>
							<label className="slds-text-heading_medium">External Id in Object</label>
						</th>
					</tr>
					{rowsdv.map(value => (
						<tr>
