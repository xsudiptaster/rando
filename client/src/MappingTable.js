...
export default class MappingTable extends Reflux.Component {
  ...
  mapFieldsAsSelectedUpdate(SheetName, SheetHeader, FieldApi) {
    ...
    for (
      var i = 0;
      i <
      this.state.ObjectDesb[this.state.objectMapping[SheetName].ObjectName]
        .fields.length;
      i++
    ) {
      if (
        this.state.ObjectDesb[this.state.objectMapping[SheetName].ObjectName]
          .fields[i].name == FieldApi &&
        this.state.ObjectDesb[this.state.objectMapping[SheetName].ObjectName]
          .fields[i].type == "reference"
      ) {
        this.state.objectMapping[SheetName].sheetObjectFields[SheetHeader][
          "ExterId"
        ] = "";
        this.state.objectMapping[SheetName].sheetObjectFields[SheetHeader][
          ...
          this.state.objectMapping[SheetName].ObjectName
        ].fields[i].referenceTo[0];
        this.state.objectMapping[SheetName].sheetObjectFields[SheetHeader][
          "RelationName"
        ] = this.state.ObjectDesb[
          this.state.objectMapping[SheetName].ObjectName
        ].fields[i].relationshipName;
        if (
  ...
  callNextSheetInsert(SheetName) {
    if (
      (this.state.objectMapping[
        SheetName
      ].sheetUpsertCalled = this.state.objectMapping[
        SheetName
    ...
    axios
      ...
      .then(response => {
        if (!response.data[0].success) {
          this.state.ErrorLog[SheetName].push(
  ...
  createTheRequestJson(indexRecord, SheetName) {
    if (this.state.objectMapping != undefined) {
      ...
      for (var i = 0; i < headerMapped.length; i++) {
        ...
        if (
          stateHeaderObject.FieldName != undefined &&
          ...
          if (
            ...
            obj[
              sheetParam.sheetObjectFields[headerMapped[i]].RelationName
            ] = InnerObj;
          } else {
  ...
  autoMatch() {
    ...
    for (var i = 0; i < sheetNames.length; i++) {
      ...
      for (
        var j = 0;
        j < this.state.objectMapping[sheetNames[i]].sheetHeaders.length;
        j++
      ) {
        var sheetHeader = this.state.objectMapping[sheetNames[i]].sheetHeaders[
          j
        ];
        for (var k = 0; k < this.state.ObjectDesb[objName].fields.length; k++) {
          var field = this.state.ObjectDesb[objName].fields[k];
          if (
            field.label.indexOf(sheetHeader) > -1 ||
            field.name.indexOf(sheetHeader) > -1
          ) {
            this.mapFieldsAsSelectedUpdate(
              sheetNames[i],
              sheetHeader,
              field.name
            );
            $(this.refs["select-" + sheetNames[i] + "-" + sheetHeader]).val(
              field.name
            );
          }
        }
    ...
    if (this.state && this.state.ObjectDesb != undefined) {
      for (var i = 0; i < Object.keys(this.state.ObjectDesb).length; i++) {
        ...
        for (
          var j = 0;
          j <
          this.state.ObjectDesb[Object.keys(this.state.ObjectDesb)[i]].fields
            .length;
          j++
        ) {
          if (
            ...
            extrDesbs[Object.keys(this.state.ObjectDesb)[i]].push(
              this.state.ObjectDesb[Object.keys(this.state.ObjectDesb)[i]]
                .fields[j]
            );
          }
        }
                            ...
                            style={{
                              display:
                                this.state.objectMapping[value]
                                  .sheetObjectFields != undefined &&
                                this.state.objectMapping[value]
                                  .sheetObjectFields[val] != undefined &&
                                this.state.objectMapping[value]
